...
{
    int m = 0;
    for (int i = 0; i < m_cMap->GetNumOfTiles_Height(); i++)//row
    {
        for (int k = 0; k < m_cMap->GetNumOfTiles_Width() + 1; k++)//col
        {
            m = tileOffset_x + k;
            ...
            if (m_cMap->theScreenMap[i][k] == 1)
            {
                ...
                Render2DMesh(meshList[GEO_STILE1], false, 1.0f,x , 575 - i*m_cMap->GetTileSize());
            }

...
void CPlayerInfo::Init(CMap* map)
{
    this->map = map;
	theHeroPosition.x=0;
	theHeroPosition.y=0;
...
void CPlayerInfo::HeroUpdate(CMap* m_cMap)
{
	...
	ConstrainHero(25, 700, 25, 575, 1.0f);
...
void CPlayerInfo::HotspotCheck()
{
    for (int i = 0; i < map->GetNumOfTiles_Height(); i++)//row
    {
        for (int k = 0; k < map->GetNumOfTiles_Width() + 1; k++)//col
        {
            //left
            //check A
            //A = 
